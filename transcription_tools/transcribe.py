    ...
    if whisperx is None:
        try:
            ...
            print(
                ...
                "Install it in the active environment before running transcription.",
                file=sys.stderr,
            )
            raise
...
def transcribe_file(
    ...
    try:
        ...
        result = wx.diarize.assign_word_speakers(diarize_segments, {"segments": segments})
        segments = result.get("segments", segments)
    except TimeoutError:
        signal.alarm(0)
...
def main():
    ...
    if diarize:
        ...
        try:
            diarize_pipeline = wx.diarize.DiarizationPipeline(
                token=hf_token,
                device=device,
            )
